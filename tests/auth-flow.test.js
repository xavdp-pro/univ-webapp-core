...
import { buildTestApp, cookieOf } from './helpers/testApp.js'
...
describe('magic-link flow', () => {
  it('request -> token -> exchange -> cookie -> protected route -> logout', async () => {
    ...
    const req = await request(app).post('/api/auth/magic/request').send({ email: ALLOWED.email })
  ...
  it('refuses a reused token', async () => {
    ...
    await request(app).post('/api/auth/magic/request').send({ email: ALLOWED.email })
    const token = mailer.lastToken()
  ...
  it('answers the same for an address that is not allowed, and sends nothing', async () => {
    const { app, mailer } = await seeded()
    const res = await request(app).post('/api/auth/magic/request').send({ email: 'stranger@example.test' })
  ...
  it('reports a mailer failure as 502 and counts it toward the limit', async () => {
    const { app, store } = await seeded({ mailer: createFakeMailer({ fail: true }) })
    const res = await request(app).post('/api/auth/magic/request').send({ email: ALLOWED.email })
    expect(res.status).toBe(502)
    expect(res.body.error).toBe('auth.sendFailed')
  ...
  it('rejects a cookie signed with another secret', async () => {
    ...
    await request(other.app).post('/api/auth/magic/request').send({ email: ALLOWED.email })
